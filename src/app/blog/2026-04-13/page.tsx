        ...
        <div className="mx-auto max-w-3xl px-6 sm:px-10 pt-24 sm:pt-36 pb-32">
          ...
          <header className="mt-12 mb-20">
            ...
            <h1 className="font-serif text-5xl sm:text-7xl leading-[0.95] tracking-tight text-ink">
              Design Patterns<br />in the Age of AI.
            </h1>
            <p className="font-serif text-2xl sm:text-3xl leading-snug mt-10 text-ink max-w-2xl">
              A lot of engineering advice is really risk management around the cost of human labor. AI changes that cost — so some of the advice is now wrong, and some is more right than before.
            </p>
          </header>

          <article className="prose-editorial font-mono">

            <h2>The economics of writing code changed</h2>
            <p>
              The cost of producing a line of working code has dropped by something like an order of magnitude in two years, and it is not going back up. AI-assisted coding is here to stay, and the current generation of models is genuinely good. Not &ldquo;good enough for a demo&rdquo; good. Good enough that a senior engineer paired with one can outship a team of five from 2022, on real systems, in production.
            </p>
            <p>
            ...
            <h2>METR&apos;s task-length benchmark</h2>
            <p>
            ...
            <figure className="my-10">
              <svg viewBox="0 0 760 420" className="w-full h-auto" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="METR 50% time horizon by model, 2019 to 2025">
                ...
                <g fill="rgb(var(--muted-rgb))" fontSize="11" fontFamily="var(--font-mono), ui-monospace, monospace" textAnchor="end">
                ...
                <g fill="rgb(var(--muted-rgb))" fontSize="11" fontFamily="var(--font-mono), ui-monospace, monospace" textAnchor="middle">
                ...
                <line x1="60" y1="344" x2="720" y2="66" stroke="rgb(var(--accent-rgb))" strokeOpacity="0.4" strokeDasharray="4 4"/>
                <text x="716" y="60" fill="rgb(var(--accent-rgb))" fontSize="10" fontFamily="var(--font-mono), ui-monospace, monospace" textAnchor="end">7-month doubling</text>
                ...
                <g fill="currentColor" fontSize="10" fontFamily="var(--font-mono), ui-monospace, monospace">
              ...
              <figcaption>
                ...
                Data: <a href="https://metr.org/blog/2025-03-19-measuring-ai-ability-to-complete-long-tasks/" target="_blank" rel="noopener noreferrer">METR, 2025</a>.
              </figcaption>
            </figure>
            ...
            <h2>Bounded contexts and inversion of control</h2>
            <p>
            ...
            <p>
              Example. A monolith has an <code>OrderService</code> that imports <code>InventoryClient</code>, <code>PaymentGateway</code>, <code>NotificationSender</code>, and <code>AuditLogger</code> directly. An AI editing that file needs to understand all four dependencies, their side effects, and their error modes. Refactored: <code>OrderService</code> lives in the orders bounded context, takes four interfaces through its constructor, and the context boundary enforces that nothing outside is directly reachable. The AI modifies order logic, runs the tests with fakes, and produces a working change without ever touching payments or inventory code.
            </p>

            <h2>Event sourcing earns its cost</h2>
            <p>
            ...
            <p>
              In return you get an auditable record of what an AI agent did to your system. A row that says a customer is on the Pro plan does not tell you which agent run upgraded them or why. A log entry that says <code>PlanUpgraded(customer=123, from=Free, to=Pro, actor=billing-agent, reason=&quot;retry of failed webhook&quot;, trace_id=...)</code> does.
            </p>
            <p>
              Example. Instead of an <code>orders</code> table you mutate in place, emit <code>OrderPlaced</code>, <code>OrderLineAdded</code>, <code>OrderDiscountApplied</code>, <code>OrderPaid</code>, <code>OrderShipped</code>. &ldquo;Current state&rdquo; is a projection. When a customer emails support asking why their total changed on Tuesday, the answer is a query against the log instead of a half-day spent correlating logs and database snapshots. The price of event sourcing is still real. It now buys something you actually want.
            </p>

            <h2>Workflow engines belong in more places</h2>
            <p>
            ...
            <h2>Rust makes more sense, for specific reasons</h2>
            <p>
              The old argument against Rust was that it slowed humans down. Borrow checker, lifetimes, exhaustive <code>Result</code> handling, three times the code of the equivalent Python. That cost was real when a human typed every character. It is a lot smaller when an AI does.
            </p>
            <p>
              The payoff went up, but only for certain kinds of AI mistakes. A strict compiler catches missing enum arms, shape mismatches, and all the &ldquo;forgot to update the other three call sites&rdquo; bugs that AI-generated patches love to introduce. It does not catch the most common hallucination, which is calling a real function that means something different from what the AI assumed. No type system saves you from that. You still need tests.
            </p>
            <p>
              Example. In a Python codebase, ask the AI to add a new <code>payment_method</code>, and it will plausibly forget one of the four places the value is switched on. In Rust with an <code>enum PaymentMethod</code> and an exhaustive <code>match</code>, the missing arm is a compile error the AI fixes on the next turn without you having to find it. The verbosity is the feature.
            </p>
            <p>
              Quick clarification so this section does not contradict itself: &ldquo;strict&rdquo; does not mean &ldquo;no metaprogramming.&rdquo; Rust has some of the heaviest metaprogramming in mainstream use, and Serde&apos;s <code>derive</code> macros are fine. The distinction is whether the metaprogramming produces typed output the compiler checks, or runtime reflection that makes behavior appear from nowhere. The first kind helps AI-written code. The second kind does not.
            </p>

            <h2>Trace everything, and don&apos;t confuse it with LLM observability</h2>
            <p>
              Logging won because <code>printf</code> was cheap and setting up OpenTelemetry was not. That economic argument is gone. You can get an AI to instrument a service with spans, attributes, and baggage propagation in an afternoon. There is almost no reason to ship a backend you cannot trace end-to-end.
            </p>
            <p>
              But tracing answers &ldquo;what happened,&rdquo; not &ldquo;why the model chose this.&rdquo; These are two different questions with two different tools. OpenTelemetry shows you the request path, the tool calls, the retries, the event writes. For the model&apos;s actual decision-making you want prompt and response capture, token-level attribution, eval harnesses, and a tool like Langfuse, Braintrust, or Phoenix hooked into the same trace IDs. Teams that treat OTel as sufficient end up with beautiful traces and no idea why the bot did what it did.
            </p>
            <p>
              Example. A customer says &ldquo;your bot refunded the wrong order.&rdquo; The OTel trace shows you the request, the tool calls, the workflow signals, and the <code>RefundIssued</code> event with its <code>reason</code> field. Langfuse shows you the exact prompt, the retrieved context, the model version, and the alternative completions the model scored lower. You need both. Either one on its own leaves you guessing in the post-mortem.
            </p>

            <h2>Heavy rewrites deserve a second look</h2>
            <p>
